...
public class Orock3OnBlockRightClickedProcedure {
	public static void execute(IWorld world, double x, double y, double z) {
		if (Math.random() <= 0.1) {
			...
			if (world instanceof World) {
				if (!((World) world).isRemote()) {
					((World) world).playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("ambient.basalt_deltas.mood")), SoundCategory.NEUTRAL, 1, 1);
				} else {
					((World) world).playSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("ambient.basalt_deltas.mood")), SoundCategory.NEUTRAL, 1, 1, false);
				}
			}
		}
	}
