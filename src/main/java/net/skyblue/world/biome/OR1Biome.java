...
public class OR1Biome {
	private static final ConfiguredSurfaceBuilder<?> SURFACE_BUILDER = SurfaceBuilder.DEFAULT
			.func_242929_a(new SurfaceBuilderConfig(SkyblueModBlocks.OC.get().getDefaultState(), SkyblueModBlocks.OC.get().getDefaultState(), SkyblueModBlocks.OC.get().getDefaultState()));
	...
	public static Biome createBiome() {
		...
		return new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(-1f).scale(0.5f).temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
