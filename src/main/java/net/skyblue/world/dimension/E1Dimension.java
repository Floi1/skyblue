...
public class E1Dimension {
	@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD)
	public static class E1SpecialEffectsHandler {
		@SubscribeEvent
		public static void registerDimensionSurfaceBuilder(FMLCommonSetupEvent event) {
			Set<Block> replaceableBlocks = new HashSet<>();
			replaceableBlocks.add(SkyblueModBlocks.ESTONE.get());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("skyblue:er_1")).getGenerationSettings().getSurfaceBuilder().get().getConfig().getTop().getBlock());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("skyblue:er_1")).getGenerationSettings().getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		...
		public static void registerDimensionSpecialEffects(FMLClientSetupEvent event) {
			...
			DeferredWorkQueue.runLater(() -> {
				try {
					...
					effectsRegistry.put(new ResourceLocation("skyblue:e_1"), customEffect);
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
	...
	public static void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("skyblue:e_1"))) {
