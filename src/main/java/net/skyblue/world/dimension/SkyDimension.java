...
public class SkyDimension {
	...
	public static class SkySpecialEffectsHandler {
		...
		public static void registerDimensionSurfaceBuilder(FMLCommonSetupEvent event) {
			...
			replaceableBlocks.add(SkyblueModBlocks.SIXTHREE.get());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("skyblue:plain")).getGenerationSettings().getSurfaceBuilder().get().getConfig().getTop().getBlock());
			replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("skyblue:plain")).getGenerationSettings().getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		...
		public static void registerDimensionSpecialEffects(FMLClientSetupEvent event) {
			...
			DeferredWorkQueue.runLater(() -> {
				try {
					...
					effectsRegistry.put(new ResourceLocation("skyblue:sky"), customEffect);
				} catch (Exception e) {
					e.printStackTrace();
				}
			});
