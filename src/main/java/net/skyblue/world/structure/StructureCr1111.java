...
public class StructureCr1111 extends ElementsSkyblueMod.ModElement {
	public StructureCr1111(ElementsSkyblueMod instance) {
		super(instance, 58);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldC1.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldC1.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 100000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("skyblue", "c1111"));
				if (template == null)
					return;
