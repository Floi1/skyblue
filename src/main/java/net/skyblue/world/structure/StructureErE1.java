...
import net.skyblue.block.BlockEgrass;
import net.skyblue.block.BlockEdirt;
...
public class StructureErE1 extends ElementsSkyblueMod.ModElement {
	public StructureErE1(ElementsSkyblueMod instance) {
		super(instance, 164);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldE1.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldE1.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 1000) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockEgrass.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = BlockEdirt.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("skyblue", "ee1"));
				if (template == null)
					return;
