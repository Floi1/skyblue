...
public class StructureGst1 extends ElementsSkyblueMod.ModElement {
	public StructureGst1(ElementsSkyblueMod instance) {
		super(instance, 205);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldG1.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldG1.NETHER_TYPE;
		}
		if (dimID == WorldG2.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldG2.NETHER_TYPE;
		}
		if (dimID == WorldG3.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldG3.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 100000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("skyblue", "gst1"));
				if (template == null)
					return;
