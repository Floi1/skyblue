...
public class StructureABBr1 extends ElementsSkyblueMod.ModElement {
	public StructureABBr1(ElementsSkyblueMod instance) {
		super(instance, 124);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldA1.DIMID) {
			...
			isNetherType = WorldA1.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 1000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("skyblue", "abbr1"));
				if (template == null)
					return;
