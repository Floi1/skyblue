...
public class StructureDDDDDR3 extends ElementsSkyblueMod.ModElement {
	public StructureDDDDDR3(ElementsSkyblueMod instance) {
		super(instance, 153);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldD1.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldD1.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("skyblue", "dddd3"));
				if (template == null)
					return;
